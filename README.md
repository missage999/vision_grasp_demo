...
### GraspController 更新摘要，[常用方法](https://chatglm.cn/share/PFcGRnlK)
1.  **接口升级**
    *   从 ROS1 风格的 `moveit.planning` 迁移到 **ROS2 原生 Action 接口** (`ActionClient` 连接 `/move_action`)，适配 ROS2 Humble 环境。
2.  **姿态约束**
    *   引入 `OrientationConstraint`，强制机械臂末端（`tool0`）在抓取过程中保持垂直向下，防止姿态随意翻转。
3.  **优化策略（混合路径约束）**
    *   **长距离移动**（预抓取/提升）：**启用**路径约束，有效防止机械臂绕圈。
    *   **短距离下降**（接近抓取）：**禁用**路径约束，避免在机械臂伸展极限处因约束过严导致规划失败。
遗留问题：1、约束没设置好。2、算法选择。3、执行速度快。4、规划时间和规划超时。5、最后返回初始位置。
